...
<form action = "putBaseballTeam" method = "post" onsubmit="return check()" name = "team">
<fieldset>
	<legend>상세 정보 입력</legend>
	팀 이름 : <input type = "text" name = "NAME"><br>
	연고지 : <select name = "TOWN">
		<option>선택하세요</option>
		<c:forEach var = "homeList" items = "${HOME }">
			<option>${homeList }</option>
		</c:forEach>
	</select><br>
	구단주 이름 : <input type = "text" name = "OWNER"><br>
	창단일 : <input type = "date" name = "BIRTH"><br>
	구단 소개 (아래) : <br>
